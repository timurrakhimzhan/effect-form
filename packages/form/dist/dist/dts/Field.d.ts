...
export declare const isArrayFieldDef: (def: AnyFieldDef) => def is ArrayFieldDef<string, Schema.Schema.Any, Schema.Schema.Any>;
...
export declare const makeArrayField: <K extends string, S extends Schema.Schema.Any, AS extends Schema.Schema.Any = Schema.Array$<S>>(key: K, itemSchema: S, modify?: (schema: Schema.Array$<S>) => AS) => ArrayFieldDef<K, S, AS>;
export type EncodedFromFields<T extends FieldsRecord> = {
    readonly [K in keyof T]: T[K] extends FieldDef<any, infer S> ? Schema.Schema.Encoded<S> : T[K] extends ArrayFieldDef<any, infer S, any> ? ReadonlyArray<Schema.Schema.Encoded<S>> : never;
};
export type DecodedFromFields<T extends FieldsRecord> = {
    readonly [K in keyof T]: T[K] extends FieldDef<any, infer S> ? Schema.Schema.Type<S> : T[K] extends ArrayFieldDef<any, infer S, any> ? ReadonlyArray<Schema.Schema.Type<S>> : never;
};
