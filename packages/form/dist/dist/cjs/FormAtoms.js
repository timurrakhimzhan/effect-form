...
var Mode = /*#__PURE__*/_interopRequireWildcard(/*#__PURE__*/require("./Mode.js"));
...
const make = config => {
  ...
  const getOrCreateFieldAtoms = fieldPath => {
    ...
    const triggerValidationAtom = fieldSchema ? runtime.fn()((value, get) => Effect.gen(function* () {
      ...
      yield* (0, _Function.pipe)(Schema.decodeUnknown(fieldSchema)(value), Effect.tap(() => Effect.sync(() => {
    ...
    const visibleErrorAtom = Atom.readable(get => {
      ...
      const isDirty = (0, _Path.isPathOrParentDirty)(dirtyFields, fieldPath);
    ...
    const onChangeAtom = Atom.fnSync()((value, get) => {
      ...
      if (shouldValidate) {
        ...
        if (parentArrayPath) {
          ...
          const arrayValue = (0, _Path.getNestedValue)(newState.values, parentArrayPath);
    ...
    const onBlurAtom = Atom.fnSync()((_, get) => {
      ...
      get.set(stateAtom, Option.some({
        ...currentState,
        touched: (0, _Path.setNestedValue)(currentState.touched, fieldPath, true)
      }));
      // Trigger validation if mode is onBlur
      if (parsedMode.validation === "onBlur") {
        const value = (0, _Path.getNestedValue)(currentState.values, fieldPath);
        get.set(triggerValidationAtom, value);
        // Also trigger parent array validation if this field is inside an array
        const parentArrayPath = getParentArrayPath(fieldPath);
        if (parentArrayPath) {
          const arrayAtoms = getOrCreateFieldAtoms(parentArrayPath);
          const arrayValue = (0, _Path.getNestedValue)(currentState.values, parentArrayPath);
          get.set(arrayAtoms.triggerValidationAtom, arrayValue);
        }
      }
    }, {
  ...
  const publicFieldAtomsRegistry = (0, _weakRegistry.createWeakRegistry)();
  const getField = field => {
    ...
    const valueAtom = Atom.readable(get => Option.map(get(stateAtom), state => (0, _Path.getNestedValue)(state.values, field.key))).pipe(Atom.setIdleTTL(0));
    ...
    const isTouchedAtom = Atom.readable(get => {
      ...
      return (0, _Path.getNestedValue)(state.value.touched, field.key) ?? false;
    }).pipe(Atom.setIdleTTL(0));
    // Typed onChange atom
  ...
  const publicArrayFieldAtomsRegistry = (0, _weakRegistry.createWeakRegistry)();
